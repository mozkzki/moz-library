...
def search_rental(params: Dict) -> Union[List[str], List[UserBookInfo]]:
    ...
    if params.get("separate", False) is False:
        ...
        _append_message(
            messages,
            message_maker.get_all_users_rental_books_message(rental_infos, params),
        )
...
def search_reserve(params: Dict) -> Union[List[str], List[UserBookInfo]]:
    ...
    for user in users:
        ...
        if params.get("separate", False):
            if params.get("mode", "reserve") == "prepare":
                _append_message(
                    messages,
                    message_maker.get_rental_and_reserved_books_message(info, params),
                )
            else:
    ...
    if params.get("separate", False) is False:
        ...
        _append_message(
            messages,
            message_maker.get_all_users_reserved_books_message(reserved_infos, params),
        )
